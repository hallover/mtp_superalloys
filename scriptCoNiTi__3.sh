...
# from last step:
## cp ../../1_scfVasp/bcc/struct_enum.out .
...
## cd ../fcc
# $scriptsFolder/enum.x
## cp ../../1_scfVasp/fcc/struct_enum.out .
## python3 $scriptsFolder/makeStr.py 1 79260 -species Co Ni Ti -config t -output=to_relax.cfg
#python3 $scriptsFolder/makeStr.py 1 7482 -species Co Ni Ti -config t -output=to_relax.cfg
...
## cd ../hcp
# $scriptsFolder/enum.x
## cp ../../1_scfVasp/hcp/struct_enum.out .
...
################################################################################
cd $workDir
mkdir 9_toMTPrelaxWithRelaxedVasp
cd 9_toMTPrelaxWithRelaxedVasp
toRelaxDir=$PWD

cp $scriptsFolder/toRelax/relax.ini .
cp $workDir/8_trainingWithRelaxedVasp/state.mvs  .
cp $workDir/8_trainingWithRelaxedVasp/pot.mtp    .
cp $workDir/4_toRelax/to_relax.cfg   .
...
echo "concatenating relaxed.cfgs..."
